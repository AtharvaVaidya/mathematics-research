...
ten.  More generally the conclusion holds in arbitrary target degree
...
We resolve this collision and all its multiples through degree ten by an
...
\(u=\infty\).  In the degree-ten basis the sole remaining polar collision
has a two-term boundary polynomial whose Wronskian is nonzero at
\(u=1\).  The finite seed-support assertions are checked by exact rational
arithmetic in the accompanying code.
...
Wronskian.  The sixth checks the cautionary next saturation.

The archived code version for this manuscript is
\begin{center}
\url{https://github.com/AtharvaVaidya/mathematics-research/tree/weighted-lift-newton-obstructions-v2}.
\end{center}
