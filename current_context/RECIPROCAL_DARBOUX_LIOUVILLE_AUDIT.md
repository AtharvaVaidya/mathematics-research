...
\tag{20a}
\]
Here \(K(\Xi)=K(T)\) is the birational endpoint lemma (or follows
directly from \(\Xi=QA(T)/(PB(T))\) in one direction).
...
There is an equally strong product form.  Put
\[
J=SY=\Xi^5T^N.
\tag{20b}
\]
...
\tag{20c}
\]
Indeed, if \(J\in K\), (15) becomes
\[
d\log Y=\frac{N\alpha}{J}\in\Omega^1_{K/\Bbbk}.
\tag{20d}
\]
In a normal closure, every \(K\)-automorphism \(\sigma\) therefore
satisfies
\[
d\log\frac{\sigma(Y)}Y=0.
\]
The constant-field hypothesis gives
\[
\sigma(T)=\eta T,\qquad \sigma(\Xi)=\varepsilon\Xi
\quad\text{for some }\eta,\varepsilon\in\Bbbk^\times.
\tag{20e}
\]
Here the second conclusion follows from
\(\sigma(\Xi)^5\sigma(T)^N=\Xi^5T^N\).
Applying \(\sigma\) to both endpoint equations and comparing the nonzero
constant coefficients of \(A,B\) gives
\(\varepsilon^2=\varepsilon^3=1\), hence \(\varepsilon=1\).
Comparing their nonzero top coefficients gives
\(\eta^p=\eta^q=1\).  Since
\[
3p-2q=1,
\]
one has \(\eta=1\).  Thus every automorphism fixes the whole branch.
The reverse implication is immediate.

Applying (20c) over \(K(J)\) strengthens (20a) once more:
\[
\boxed{\qquad
K(J)=K(\Xi)=K(T)=K(S)=K(Y).
\qquad}
\tag{20f}
\]
In the original radial variables,
\[
J=(ZW^m)^5(W^{-1})^{5m+2}=\frac{Z^5}{W^2}.
\tag{20g}
\]
Under \(X=Z^2/W,\ Y_{\rm old}=W/Z\), this is the simple monomial
\(X^3Y_{\rm old}\).  The global section problem may therefore be attacked
by trying to descend this single monomial rather than either Darboux
...
Darboux powers \(S,Y\), or their product \(J\).
...
The product \(J=SY\) does not descend either.  Otherwise (20d)--(20e)
and the same constant/top-coefficient comparison would force
\(\Xi,T\in K\), contradicting the totally ramified degree-\(N\)
extension.  This product argument does not use the trace step or finite
generation of the uncompleted base.
...
Consequently the precise remaining bridge can be stated in any of three
equivalent forms:
...
> Keller completion, any one of
> \[
> T^N,\qquad \Xi^5,\qquad \frac{Z^5}{W^2}
> \]
> belongs to \(\Bbbk(z,w)\).

By (16) and (20c), each version is already the full field-descent
statement.  The completed-local construction (28)--(37) proves that it
cannot follow
