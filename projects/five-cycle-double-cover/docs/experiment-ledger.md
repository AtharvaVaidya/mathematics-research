...
## Exact rooted cut relations and abstract countermodels

Audit date: **2026-08-01**.

The package `scratch/d5-cut-signature-natural-join-20260801/` passed all
three retained runners and its SHA-256 manifest.  The direct semantic replay
checks 324 normalized whole flows for a two-cut and 27 for a three-cut,
including equality with the Cartesian gluing of the two shore-flow sets and
the external-mask intersection formula.  The connected-shore census through
host order ten found:

| cut | root relations | cap relations | crossed pairs | bad |
|---:|---:|---:|---:|---:|
| 2 | 7 | 9 | 63 | 0 |
| 3 | 108 | 222 | 23,976 | 0 |

The separate complete abstract audit quotients by the residual boundary
stabilizer.  For two-cuts it obtains 13 root-state orbits, 51 cap-state
orbits, and 281 incompatible orbit edges; for three-cuts it obtains 138,
922, and 74,985 respectively.  Minimal nonvacuous unsafe relation pairs are
frozen and satisfy all implemented parity, local cap, and symmetry axioms.
They are countermodels to a local-axiom induction, not graph realizations
and not FiveCDC counterexamples.

## Complement-circuit external-reentry frontier

Audit date: **2026-08-01**.

`scratch/d5-complement-circuit-external-frontier-20260801/run_all.sh`
reproduced the complete biconnected simple cubic censuses at orders 12 and
14, the focused full-cycle-space/depth-two audit, and a separately
structured literal replay.  The exact order-14 totals are:

| quantity | count |
|---|---:|
| normalized flows modulo global `S5` | 537,418 |
| ordinary Kempe orbits | 33,102 |
| orbit/rooted-interface pairs | 8,341,704 |
| bad ordinary-orbit interfaces | 72 |
| immediate simple-complement rescues | 60 |
| strict one-Eulerian-support failures | 12 |
| disjoint complement edges to a good Kempe orbit | 12/12 |

Every strict row also has a direct depth-two complement rescue.  A standalone
checker reconstructs one 432-state bad ordinary orbit and a disjoint
five-circuit followed by four displayed Kempe switches.  The package
manifest and expected/reproduced transcript pass.  The strict graphs are
Tait-colourable and have girth at most four; the computation is a sharp
strategy delimiter, not a FiveCDC obstruction.

## Certificate-checked rooted-interface extension

Audit date: **2026-08-01**.

The primary streaming search and separately written semantic checker in
`scratch/d5-rooted-interface-frontier-through34-20260801/` agree on the
following positive certificate totals:

| input | graphs | interfaces | retained flow states |
|:--|--:|--:|--:|
| complete biconnected simple cubic order 16 | 3,874 | 1,301,664 | 65,185 |
| retained canonical cyclically-4 non-Tait orders 18--26 | 1,491 | 1,361,316 | 57,899 |
| seven literal retained strong-snark order-34 rows | 7 | 11,424 | 997 |
| **total** | **5,372** | **2,674,404** | **124,081** |

Every interface has an aggregate typed double star, aggregate external
coverage of all three cap ports, and one flow with simultaneous external
coverage.  The checker independently decodes graph6, verifies weight-two
labels and all vertex xors, reconstructs factor components, checks every
interface, and verifies the stated graph properties.  The order-16 stream
is regenerated by `geng`; higher complete-corpus wording is conditional on
the retained Snarkhunter provenance, and the order-34 file is explicitly
nonexhaustive.  This is finite positive evidence, not a FiveCDC resolution.
