...
through size thirteen, and gives exact finite censuses.  It proves neither the
...
extendable projection of size at most thirteen is cleanable, and a
counterexample to the proposed selection principle must have minimum size
at least fourteen.  This boundary method is sharp: at size fourteen an
explicit \(7+7\) state is neither directly cleanable nor circuit-deletable.
...
dichotomy, not to the minimum-projection principle or FiveCDC.
...
\begin{enumerate}
  ...
  \item a human proof through size five and an exact boundary extension
        through size thirteen, plus a sharp size-fourteen counterstate
        to the unrestricted boundary method;
...
\subsection{Clean or delete at sizes twelve and thirteen}
...
the example is global minimality: a different size-five projection exists.
The remaining argument must therefore use the full four-colour exchange
inequalities, not only their consequence that every support circuit is
four-coloured.
...
given here.  By Theorem~\ref{thm:sizethirteen}, any counterexample to this
selection principle has minimum support at least fourteen.
...
\ref{thm:sizethirteen} close every possible
nonzero support size through thirteen:
a counterexample to Conjecture~\ref{conj:minimum} must have minimum
projection size at least fourteen.
...
for the clean-or-delete boundary method.  Its realization also shows why
the exchange theorem cannot be discarded: boundary conservation and
four-colour use alone admit an uncleanable projection, while a smaller
cleanable projection elsewhere in the same graph saves the conjecture.
...
The size-fourteen package independently checks the first failure of the
unrestricted dichotomy, its 40-case map certificate, and its exhaustive
18-vertex realization analysis.
