...
isolates a new candidate selection principle.  It proves neither the
...
We then perform an exact census of all \(41{,}301\) connected simple cubic
graphs of order \(18\).  Among the \(39{,}866\) bridgeless graphs, exactly
\(179\) are not Tait-colourable.  Full cycle-space enumeration shows that
every minimum-cardinality extendable projection of every one of these
\(179\) graphs has a clean lift satisfying the Hu\v{s}ek--\v{S}\'amal
component parity condition.  The canonical record digest is
...
\begin{enumerate}
  ...
  \item elementary consequences for four simultaneous shortest
        \(T\)-joins;
  \item a reproducible exhaustive order-\(18\) census; and
...
\section{Exact order-\(18\) census}\label{sec:census}
...
enumeration; it is still a computational result and should be independently
rerun.
