...
through size five, and gives exact finite censuses.  It proves neither the
...
We also prove, without computation, that every globally minimum
...
non-Tait-colourability.  Thus a counterexample to the proposed
minimum-projection selection principle must have minimum size at least six.
...
\begin{enumerate}
  ...
  \item a human-checkable theorem settling minimum projection sizes at
        most five;
...
Theorem~\ref{thm:sizefive} leaves the minimum-size-six boundary open.
\end{remark}
...
Theorem~\ref{thm:sizefive} closes the first two possible nonzero support
sizes: a counterexample to Conjecture~\ref{conj:minimum} must have minimum
projection size at least six.
...
minimum-projection exchange proof and the size-at-most-five proof are
displayed in full so that they can be checked without trusting an AI
system.  The order-\(18\) result is
