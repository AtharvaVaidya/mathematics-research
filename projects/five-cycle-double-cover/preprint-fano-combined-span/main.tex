...
a universal span identity and a Petersen obstruction}
...
\colorbox{yellow!22}{%
  ...
  derived the two-cycle normal form and the Petersen example, wrote the
  search and checking programs, and drafted this manuscript under human
  direction.  The checkers and the present proof exposition are themselves
...
\[
 ...
 G\text{ is not Tait-colourable},                       \tag{5.1}
\]
...
inequality in (5.1) implies that one of the four affine colours is absent.
...
\end{gathered}                                           \tag{6.2}
\]
Each triple has XOR zero, so (6.1) is a nowhere-zero
...
For the flow (6.1), neither \(L=\{1,2,3\}\) nor
...
vertices.  Contracting them in the graph (6.1), the remaining ten edges
...
check, its perfect matchings in the edge indexing (6.1) are exactly
\[
\begin{gathered}
...
(6.5).  Direct deletion shows that the complement of every matching in
(6.5) is two 5-cycles.  In a Tait colouring, one colour class is a
...
\[
                            10<4\cdot3.
\]
Theorem~\ref{thm:obstruction} applied to (6.3) and (6.4) proves both
claims.
\end{proof}

\begin{remark}[not a 5-CDC counterexample]
The following five Eulerian edge-id sets cover every edge of (6.1)
exactly twice:
\[
\begin{aligned}
&\{0,1,3,4,7\},\\
...
directly from (6.1).  Thus (6.6) is a positive 5-CDC of the example.
\end{remark}
...
perfect matchings in (6.5).  The six nonzero failures have rank five and
...
above and keyword/full-text searches through 26 July 2026.  We did not
...
obstruction in those sources.  We therefore regard these as
...
\begin{enumerate}
  ...
  \item a structural contraction obstruction, instantiated by a small
        fully checkable fixed-projection counterexample.
\end{enumerate}
