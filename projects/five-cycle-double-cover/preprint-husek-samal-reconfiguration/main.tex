...
not Tait-colourable.  For the displayed nowhere-zero flow, a
standard-library checker exhausts all \(8{,}797\) simple cycles and all
\(1{,}604\) legal cycle--value switches; none reaches the component
condition.  An explicit two-switch path does reach it, proving that the
displayed flow has exact distance two.  An independently checked
...
\item an exact strict 26-vertex countermodel to the assertion that one
      simple-cycle switch always suffices, together with an exact
      distance-two path and a positive FiveCDC certificate; and
...
\begin{equation}
\begin{split}
(&7,5,3,7,1,3,2,3,5,5,2,6,1,4,7,2,3,7,6,5,\\
 &4,1,6,4,4,6,2,5,6,4,2,1,4,3,3,2,6,7,1).
\end{split}
\end{equation}
...
\item The displayed values form a nowhere-zero \(W\)-flow with profile
      \[
                           (8,8,4,6,6,6,4).
      \]
\item The graph has \(8{,}797\) simple cycles and exactly \(1{,}604\)
...
\begin{array}{c@{\qquad}l}
 a=1&
 C=(1,2,3,7,8,9,10,11,13,15,16,19,20,27,28,29),\\[2mm]
 a=5&
 C=(0,1,2,4,6,7,8,9,10,11,15,30,32,33).
\end{array}
\end{equation}
...
The resulting exact totals are \(8{,}797\) cycles, \(1{,}604\) legal
switches, and zero clean neighbours.  This proves
\(\dist_{\HS}(f)\geq2\).  Direct verification of (5.3)--(5.4) proves
\(\dist_{\HS}(f)\leq2\).

Finally, the checker tests every weight and every vertex-coordinate
parity in (5.2).  These finite checks prove all five assertions.
\end{proof}
...
\midrule
26 & \(8{,}797\)   & \(1{,}604\)  & \(4\) & \(2\) & no repair; distance \(2\)\\
...
\nolinkurl{1c8c0b5959fb7e8f7333b44860a519e1351f366e0a78ab5be8a061287f9dfd46}
& order-26 retained state\\
...
not evidence of a universal radius-two theorem.  Likewise, the
packing-to-switch lemma resolves states possessing an appropriate
packing by a finite sequence; it supplies no universal packing theorem.
