#define main rooted_three_pole_cadical_original_main
#include "rooted_three_pole_cadical.cpp"
#undef main
...
SatBasePairDecision screen_root_sat(RootedSat& solver, int root) {
  std::array<int, 7> decision{};
  decision.fill(-1);
  SatBasePairDecision result;
  ...
  auto query = [&](int orbit) {
    ...
    for (int label = 0; label < 10; ++label) {
      if (orbit_of(solver.values()[label]) != orbit) {
        continue;
      }
...
int main(int argc, char** argv) {
  ...
  if (argc == 2 && std::string(argv[1]) == "--transcript") {
    ...
    std::cerr << "usage: rooted_base_pair_screen_cadical "
  ...
  while (std::getline(std::cin, record)) {
    ...
    try {
      ...
      for (int root = 0; root < static_cast<int>(graph.edges.size()); ++root) {
        if (bridge_without_semiedges(graph, root)) {
          continue;
        }
        ++nonbridge_roots;
        const auto decision = screen_root_sat(solver, root);
  ...
  std::cerr << "SUMMARY implementation=cadical graphs=" << graphs
