#!/usr/bin/env python3
"""Independent verifier for the complete retained order-26 cyclic-4 probe."""
...
EXPECTED_SOURCE_ROWS = 1_297
EXPECTED_SOURCE_SHA256 = (
    "1d2b95b9d412f5f6b8ccb788779bd685df23b3239bda8c7e9557266375519760"
)
EXPECTED_POLE_ROWS = 859_911
EXPECTED_POLE_SHA256 = (
    "8e346b040fb4c69bef5ab6b21b30c30635a22ba4086a9b5ea567cdf3d10378e4"
)
EXPECTED_TABLE_SHA256 = (
    "abc46bc0af6f94596ae11c68af95acc55cf8d73d82f81aea4a06ab8614a14da2"
)
EXPECTED_SHARD_ROWS = (107_489,) * 7 + (107_488,)
SUMMARY = re.compile(
...
def main() -> int:
    ...
    if (
        "snarkhunter 26 4 S s C4 o g" not in generator_log
        or "All done, 1297 graphs generated with 26 vertices."
        not in generator_log
    ):
        raise SystemExit("malformed Snarkhunter log")
    if (ARTIFACTS / "expand.log").read_text(encoding="utf-8") != (
        "SUMMARY graphs=1297 poles=859911\n"
    ):
        raise SystemExit("malformed pole-expansion log")

    source = (ARTIFACTS / "cyclic4-nontait-order26.g6").read_bytes()
    ...
    poles = gzip.decompress(
        (ARTIFACTS / "cyclic4-nontait-order26-poles.g6.gz").read_bytes()
    )
    ...
    result = {
        "schema": "five-cdc-order26-cyclic4-cap-signatures-v1",
        ...
        "result": (
            "Every independent-edge deletion pole from the retained 1,297 "
            "cyclically 4-edge-connected non-Tait order-26 caps realizes "
            "all ten fixed-five D5 boundary types."
        ),
        "warning": (
            "Completeness of the 1,297-graph cyclic-4 source relies on the "
