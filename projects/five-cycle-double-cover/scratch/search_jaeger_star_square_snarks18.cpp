...
//     pickg -q -g5 -GG3 |
//     pickg -q -A2 |
//     ./search_jaeger_star_square_snarks18 > witnesses.tsv
//
...
Graph parse_graph6(std::string record) {
  ...
  if (graph.n != 18) throw std::runtime_error("expected order 18");
  std::vector<int> bits;
...
std::vector<Mask> omission_options(
  ...
  if (internal_edges.size() != 24) {
    throw std::runtime_error("wrong number of nonroot edges");
  }
  // Generate only the C(24,8) relevant words, rather than scanning 2^24.
  std::array<int, 8> chosen{};
  const auto visit = [&](const auto& self, int depth, int next) -> void {
    if (depth == 8) {
      Mask omitted = 0;
      for (const int slot : chosen) {
        omitted |= Mask{1} << internal_edges[slot];
      }
      const Mask tree =
          (internal ^ omitted) | (Mask{1} << root_spoke);
      if (is_tree(graph, tree)) result.push_back(omitted);
      return;
    }
    for (int slot = next; slot <= 24 - (8 - depth); ++slot) {
      chosen[depth] = slot;
      self(self, depth + 1, slot + 1);
    }
  };
  visit(visit, 0, 0);
  return result;
...
int main(int argc, char** argv) {
  ...
  if (records != 3) {
    std::cerr << "expected 3 canonical records, received " << records
              << '\n';
    return 1;
  }
  std::cerr
