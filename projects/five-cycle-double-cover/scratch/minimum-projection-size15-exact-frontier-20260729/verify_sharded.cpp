...
static bool has_removable_repeated_position(const Word& word,
                                            int offset,
                                            int length) {
    std::array<int, 4> multiplicity{};
    for (int local = 0; local < length; ++local)
        ++multiplicity[word[offset + local]];
    for (int local = 0; local < length; ++local) {
        int previous = offset + (local + length - 1) % length;
        int next = offset + (local + 1) % length;
        int current = offset + local;
        if (multiplicity[word[current]] >= 2 &&
            word[previous] != word[next])
            return true;
    }
    return false;
}
...
struct Auditor {
    ...
    std::array<std::array<unsigned char, 6>, 7> closure_contribution{};
    Counts counts;
    ...
    void set_closure_contributions() {
        for (int component = 0; component < components; ++component) {
            for (int map_index = 0; map_index < (int)GL.size(); ++map_index) {
                unsigned char encoded = 0;
                for (int circuit = 0;
                     circuit < (int)shape.lengths.size(); ++circuit) {
                    unsigned char value = 0;
                    int offset = shape.offsets[circuit];
                    int length = shape.lengths[circuit];
                    for (int local = 0; local < length; ++local) {
                        int edge = offset + local;
                        if (partition[edge] == component)
                            value ^= GL[map_index][derivative[edge]];
                    }
                    encoded |= value << (2 * circuit);
                }
                closure_contribution[component][map_index] = encoded;
            }
        }
    }

    std::pair<bool, bool> map_search(int component,
                                     unsigned char closure,
                                     bool deletion_seen) {
        if (component == components) {
            if (closure != 0) return {false, deletion_seen};
            auto [clean, deletable] = evaluate_maps();
            return {clean, deletion_seen || deletable};
        }
        bool seen = deletion_seen;
        for (int map_index = 0; map_index < (int)GL.size(); ++map_index) {
            maps[component] = GL[map_index];
            auto [clean, deletable] =
                map_search(component + 1,
                           closure ^ closure_contribution[component][map_index],
                           seen);
    ...
    void classify_partition() {
        ...
        ++counts.dirty;
        set_closure_contributions();
        auto [clean, deletable] =
            map_search(1, closure_contribution[0][0], false);
        if (clean) {
            ++counts.direct_clean;
        } else if (deletable) {
...
static void run_shape(const Shape& shape, const std::string& name,
    ...
    for (const auto& word : words) {
        for (int circuit = 0; circuit < (int)shape.lengths.size(); ++circuit) {
            if (shape.lengths[circuit] >= 5 &&
                !has_removable_repeated_position(
                    word, shape.offsets[circuit], shape.lengths[circuit]))
                throw std::runtime_error("removable-position lemma failed");
        }
    }
