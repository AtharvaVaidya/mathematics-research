...
struct ParityProfile {
  ...
  int two_direction_planes = 0;
  int full_h_soluble_planes = 0;
  int full_h_total_rank = 0;
  int full_h_total_cokernel = 0;
};

bool in_binary_span(uint64_t target, std::vector<uint64_t> columns) {
  std::array<uint64_t, 64> basis{};
  for (uint64_t column : columns) {
    while (column) {
      const int pivot = 63 - __builtin_clzll(column);
      if (!basis[pivot]) {
        basis[pivot] = column;
        break;
      }
  ...
  while (target) {
    const int pivot = 63 - __builtin_clzll(target);
...
int binary_span_rank(std::vector<uint64_t> columns) {
  std::array<uint64_t, 64> basis{};
  int rank = 0;
  for (uint64_t column : columns) {
    while (column) {
...
bool full_h_circuit_switch_soluble(
    const Graph& graph, const DirectFlow& flow, int plane_mask,
    int* output_rank = nullptr, int* output_cokernel = nullptr) {
  Dsu components(graph.n);
  std::vector<int> h_degree(graph.n);
  std::vector<int> leaf_value(graph.n);
  ...
  for (int edge = 0; edge < graph.m(); ++edge) {
    ...
    if (plane_mask & (1 << value)) {
      ++h_degree[left];
      ++h_degree[right];
      leaf_value[left] ^= value;
      leaf_value[right] ^= value;
      const int left_root = components.find(left);
      const int right_root = components.find(right);
      if (left_root != right_root) {
        components.parent[left_root] = right_root;
      }
    } else {
      outside_edges.push_back(edge);
    }
  }
  std::map<int, int> component_index;
  for (int vertex = 0; vertex < graph.n; ++vertex) {
    assert(h_degree[vertex] == 1 || h_degree[vertex] == 3);
    const int root = components.find(vertex);
    if (!component_index.count(root)) {
      const int next = static_cast<int>(component_index.size());
      component_index[root] = next;
    }
  }
  const auto component_of = [&](int vertex) {
    return component_index.at(components.find(vertex));
  };

  int outside_value = 1;
  while (plane_mask & (1 << outside_value)) ++outside_value;
  uint64_t beta = 0;
  for (const int edge : outside_edges) {
    if (direct_flow_value(flow, edge) != outside_value) continue;
    const auto [left, right] = graph.edges[edge];
    beta ^= uint64_t{1} << component_of(left);
    beta ^= uint64_t{1} << component_of(right);
  }

  std::vector<std::vector<std::pair<int, int>>> outside_adjacency(
      graph.n);
  for (const int edge : outside_edges) {
    const auto [left, right] = graph.edges[edge];
    outside_adjacency[left].push_back({right, edge});
    outside_adjacency[right].push_back({left, edge});
  }
  std::vector<int> seen_edge(graph.m());
  std::vector<uint64_t> columns;
  for (const int first_edge : outside_edges) {
    if (seen_edge[first_edge]) continue;
    std::vector<int> circuit_vertices;
    std::vector<int> stack{first_edge};
    while (!stack.empty()) {
      const int edge = stack.back();
      stack.pop_back();
      if (seen_edge[edge]) continue;
      seen_edge[edge] = true;
      const auto [left, right] = graph.edges[edge];
      circuit_vertices.push_back(left);
      circuit_vertices.push_back(right);
      for (const int vertex : {left, right}) {
        for (const auto [other, adjacent_edge] :
             outside_adjacency[vertex]) {
          (void)other;
          if (!seen_edge[adjacent_edge]) stack.push_back(adjacent_edge);
        }
      }
    }
    std::sort(circuit_vertices.begin(), circuit_vertices.end());
    circuit_vertices.erase(
        std::unique(circuit_vertices.begin(), circuit_vertices.end()),
        circuit_vertices.end());
    for (int direction = 1; direction < 8; ++direction) {
      if (!(plane_mask & (1 << direction))) continue;
      uint64_t gamma = 0;
      for (const int vertex : circuit_vertices) {
        assert(h_degree[vertex] == 1);
        if (leaf_value[vertex] != direction) {
          gamma ^= uint64_t{1} << component_of(vertex);
        }
      }
      columns.push_back(gamma);
    }
  }
  const int rank = binary_span_rank(columns);
  if (output_rank != nullptr) *output_rank = rank;
  if (output_cokernel != nullptr) {
    *output_cokernel =
        static_cast<int>(component_index.size()) - rank;
  }
  return in_binary_span(beta, columns);
}

ParityProfile parity_profile(const Graph& graph, const DirectFlow& flow) {
  ParityProfile profile;
  std::map<int, int> parallel_plane_counts;
  for (const auto& pair_choice : PAIR_ORBITS) {
    const auto result = six_hole_system(graph, flow, pair_choice);
    const bool parallel =
        (pair_choice[0] ^ pair_choice[1]) ==
    ...
    if (parallel) {
      ...
      parallel_plane_counts.try_emplace(direction_plane_mask, 0);
      if (result.soluble) {
        ++parallel_plane_counts[direction_plane_mask];
      }
    }
  ...
  // Regression guard: full-H must inspect all seven planes, including a
  // plane for which none of its three single-direction systems is soluble.
  assert(parallel_plane_counts.size() == 7);
  for (const auto& [plane, count] : parallel_plane_counts) {
    if (count == 2) ++profile.two_direction_planes;
    int full_h_rank = 0;
    int full_h_cokernel = 0;
    if (full_h_circuit_switch_soluble(
            graph, flow, plane, &full_h_rank, &full_h_cokernel)) {
      ++profile.full_h_soluble_planes;
    }
    profile.full_h_total_rank += full_h_rank;
    profile.full_h_total_cokernel += full_h_cokernel;
  }
  return profile;
}
