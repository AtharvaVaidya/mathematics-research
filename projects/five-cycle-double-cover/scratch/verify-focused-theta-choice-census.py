...
def audit_order(order: int, limit: int) -> dict[str, object]:
    corpus = CORPORA[order]
    corpus_bytes = corpus.read_bytes()
    digest = hashlib.sha256(corpus_bytes).hexdigest()
    if digest != EXPECTED_HASHES[order]:
        raise AssertionError(f"order-{order} corpus hash changed")
    rows = corpus_bytes.decode("ascii").splitlines()
    if limit:
        rows = rows[:limit]
    totals = {
        "pairs": 0,
...
def main() -> None:
    ...
    if any(order not in CORPORA for order in orders):
        raise AssertionError("orders must be even and between 10 and 28")

    started = time.time()
    results = [audit_order(order, args.limit) for order in orders]
    output = {
        "classification": "PASS",
        "warning": (
