// Search fixed three-tree multiplicity fibres for a good Oum pair labelling
// supported on at most six of the eight coordinate points.
...
Encoding encode_six_point_good(const Graph& graph) {
  ...
  for (int omitted = 0; omitted < 8; ++omitted) {
    std::vector<int> row;
    for (int point = 0; point < 8; ++point) {
      if (point != omitted) row.push_back(-used[point]);
    }
    formula.clause(row);
  }
  return result;
...
void print_defect_six(const std::vector<int>& multiplicity) {
  std::cout << '[';
  bool first = true;
...
int main(int argc, char** argv) {
  if (argc != 2) {
    std::cerr << "usage: search_jaeger_six_point_fibre_sat GRAPH6\n";
    return 64;
  }
  ...
  const Encoding six_encoding = encode_six_point_good(graph);
  Incremental feasibility(feasibility_encoding.formula);
  Incremental six(six_encoding.formula);
  ...
  const auto audit = [&](char type) {
    ...
    ++feasible;
    const int six_status = six.solve(six_encoding, multiplicity);
    assert(six_status == 10 || six_status == 20);
    if (six_status == 10) {
      ++successful;
      verify_good_model(graph, six_encoding, six, multiplicity);
      return false;
    }
    const auto trees = extract_trees(feasibility_encoding, feasibility);
    std::cout << "{\"status\":\"NO_SIX_POINT_GOOD_REPRESENTATIVE\","
              << "\"graph6\":\"" << graph6
              << "\",\"vertices\":" << graph.n
              << ",\"edges\":" << graph.m()
              << ",\"type\":\"" << type << "\",\"defect\":";
    print_defect_six(multiplicity);
  ...
  std::cout << "{\"status\":\"GRAPH_DONE\",\"graph6\":\""
            ...
            << ",\"six_point_good_fibres\":" << successful
            << ",\"failures\":0}\n";
  return 0;
}
