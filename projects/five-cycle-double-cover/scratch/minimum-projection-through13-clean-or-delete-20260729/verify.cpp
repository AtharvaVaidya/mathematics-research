...
static std::string normalized_partition(
    const std::array<unsigned char, N>& partition,
    const std::vector<int>& indices) {
    std::array<int, N> names;
    names.fill(-1);
    int next = 0;
    std::string result;
    for (int index : indices) {
...
struct Auditor {
    ...
    std::array<unsigned char, N> certificate_base{};
...
int main(int argc, char** argv) {
    ...
    run_shape(Shape{{4, 4, 5}, {0, 4, 8}}, "4+4+5");
    run_shape(Shape{{4, 9}, {0, 4}}, "4+9");
    run_shape(Shape{{5, 8}, {0, 5}}, "5+8");
    run_shape(Shape{{6, 7}, {0, 6}}, "6+7");
    run_shape(Shape{{13}, {0}}, "13");
}
