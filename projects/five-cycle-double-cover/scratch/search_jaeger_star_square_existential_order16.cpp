...
// on simple 3-edge-connected cubic graphs of order 16.
//
// Generate the canonical input with:
//
//   geng -cq -d3 -D3 16 |
//     ./search_jaeger_star_square_existential_order16 > witnesses.tsv
//
...
Graph parse_graph6(std::string record) {
  ...
  if (graph.n != 16) throw std::runtime_error("expected order 16");
  std::vector<int> bits;
...
std::vector<Mask> omission_options(
  ...
  if (internal_edges.size() != 21) {
    throw std::runtime_error("wrong number of nonroot edges");
  }
  for (unsigned word = 0; word < (1U << 21); ++word) {
...
int main(int argc, char** argv) {
  ...
  if (records != 4060) {
    std::cerr << "expected 4060 canonical records, received " << records
              << '\n';
    return 1;
  }
  std::cerr
