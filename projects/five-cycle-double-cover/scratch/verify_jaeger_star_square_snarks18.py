#!/usr/bin/env python3
"""Independently verify the square witnesses for all order-18 snarks.

This checker does not import or call the C++ discovery program.  It obtains
the canonical connected cubic order-18 stream from nauty ``geng``, checks a
frozen SHA-256 digest, and independently identifies the three snarks by
testing three-edge-connectivity, girth at least five, and non-three-edge-
colourability.  It then requires exactly one witness for every labelled root
and eligible independent edge pair of each snark.  Every downstairs and
lifted spanning-tree triple, multiplicity condition, outside-edge agreement,
and odd-kernel component-parity condition is recomputed from the integer
masks in the corpus.
"""
...
def girth_at_least_five(
    ...
    for vertex in range(vertex_count):
        neighbours = tuple(adjacency[vertex])
        for first in range(len(neighbours)):
            for second in range(first + 1, len(neighbours)):
                left = neighbours[first]
                right = neighbours[second]
                if right in adjacency[left]:
                    return False
                if (adjacency[left] & adjacency[right]) - {vertex}:
                    return False
    return True


def is_three_edge_colourable(
    vertex_count: int, edges: tuple[tuple[int, int], ...]
) -> bool:
    colours = [-1] * len(edges)
    used = [0] * vertex_count
    ...
    def search(coloured: int) -> bool:
        ...
        while best_available:
            bit = best_available & -best_available
            best_available ^= bit
            colours[best_edge] = bit.bit_length() - 1
            used[left] |= bit
            used[right] |= bit
...
def main() -> None:
    ...
    all_graphs = tuple(decode_graph6(record) for record in records)
    snark_records = tuple(
        record
        for record, graph in zip(records, all_graphs)
        if girth_at_least_five(*graph)
        and is_three_edge_connected(*graph)
        and not is_three_edge_colourable(*graph)
    )
    assert snark_records == EXPECTED_SNARKS
    graphs = tuple(decode_graph6(record) for record in snark_records)
    retained = set(range(len(graphs)))

    # A compact bit set avoids retaining millions of Python pair tuples.
    # Bit ``first * 27 + second`` represents one eligible edge pair.
    ...
    assert witness_count == 3 * 18 * 231 == 12_474
    print(
        "PASS: independently verified all order-18 snark square instances:",
        f"canonical_records={len(records)}",
        f"canonical_sha256={CANONICAL_SHA256}",
        f"snarks={len(retained)}",
