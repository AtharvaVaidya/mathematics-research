...
all biconnected simple cubic graphs through 14 vertices.  At order 14 this
checks 33,598 terminal plateaus and 642,167 fixed-distance subplateaus.  A
human-checkable first-foreign factor-splice lemma refines the distance by a
cyclic blocker statistic; two implementations again find no obstruction
through order 14.  These are finite results only.  The universal
terminal-plateau statement is left explicitly as a conjecture.
\end{abstract}
...
At order 14, the stronger terminal-plateau chain test was run completely.
For fixed roots \(r,s\), let \(d_q(r,s)\) be the minimum number of factor
components in an intersection chain from \(r\) to \(s\).  Thus \(d=1\)
is exactly the root-good condition.  Within every terminal
\(\Phi\)-plateau, split the states of each fixed bad value \(d>1\) into
components under neutral moves preserving \(d\).  Every such component
had a neutral boundary edge to smaller distance:
\[
\begin{array}{lr}
\toprule
...
\text{fixed-\(d>1\) subplateaus} &642{,}167\\
\text{maximum factor-component-chain distance} &3\\
\text{failed fixed-distance subplateaus} &0\\
\bottomrule
\end{array}                                                  \tag{13}
\]
An independently structured Python implementation agrees through order
12 on 2,659 terminal plateaus and 33,610 fixed-distance subplateaus, again
with no failure.  Equation (13) is finite evidence only; it does not prove
the universal closed-subplateau exit statement.

\subsection{A first-foreign splice and a refined finite target}

For a coordinate pair \(P\), write \(Y_P\) for its factor.  Let \(P,Q\)
share exactly one coordinate, put \(P'=P\mathbin\triangle Q\), and let
\(H\) be a component of \(Y_Q(q)\).  If \(q'\) is obtained by switching
\(Q\) on \(H\), then the following edgewise identity holds:
\[
             Y_{P'}(q')
             =Y_P(q)\mathbin\triangle\bigl(Y_Q(q)\setminus H\bigr).
                                                               \tag{14}
\]
Outside \(H\), the indicator of \(Y_{P'}\) is the sum modulo two of
the indicators of \(Y_P,Y_Q\).  On \(H\), transposing \(Q\) converts
membership in \(Y_{P'}\) to membership in \(Y_P\), proving (14).

Let \(C\) be the \(Y_P\)-component through a root \(r\), and let \(D\ne H\)
be a \(Y_Q\)-component.  Orient \(C\) from \(r\), ignoring all returns to
\(H\).  If the first component of \(Y_Q\setminus H\) encountered is \(D\),
then the new \(Y_{P'}\)-component through \(r\) meets \(D\).  Indeed, the
initial segment of \(C\) survives in (14), the common path with \(D\)
cancels at the first encounter, and cubicity pairs the incoming \(C\)-edge
with the outgoing \(D\)-edge.  For a distance-two chain \(C,D\), this
proves that the switch does not increase the rooted distance.

The identity suggests counting maximal foreign \(Y_Q\)-component blocks
encountered before \(D\).  Minimizing this count over both orientations
and all eligible shared-coordinate shortest first pairs defines \(b^*\);
when no eligible pair exists, \(b^*=0\), meaning only that this particular
obstruction is absent.  Complete fixed-\((d,b^*)\) terminal-subplateau
censuses through order 14 gave zero failures.  The primary
lower-index-root implementation found 646,399 subplateaus, while a
separate directed-root implementation found 1,293,664.

The refinement is not a local greedy theorem.  A seven-state terminal
plateau on 12 vertices has a neutral prescribed switch with
\(d:2\to2\), not \(2\to1\).  A separate order-14 terminal witness has
\((d,b^*)=(2,1)\), but every switch of the minimizing root component or
first blocker either decreases \(\Phi\) or raises \(d\); a neutral switch
on a third coordinate pair reaches \(d=1\).  Thus the remaining universal
obligation is a global neutral closed-cage exclusion, not a one-switch
rule.  In that witness the unused fifth coordinate gives a transparent
multi-splice, but this is not universal either: an exact order-12 terminal
state with \(b^*=1\) uses all five coordinates.
...
terminal-chain and blocker verifiers check the frozen complete order-14
reports, summary arithmetic, failure fields, and cryptographic hashes.
The directed-root blocker implementation is separately structured and has
its own frozen summaries and hash ledger.  These checks certify finite
computations only.
...
\begin{enumerate}
 ...
 \item the first-foreign factor-splice identity and its distance
 nonincrease consequence.
\end{enumerate}
...
orientable FiveCDC result has been obtained.  The terminal-plateau and
closed-subplateau exit statements are proof targets suggested by the
